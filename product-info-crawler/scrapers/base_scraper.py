...
class BaseScraper:
    ...
    def parse_category(self, category_name: str, url: str) -> list[dict]:
        ...
        try:
            ...
            self._wait_for_content(scraping_settings)
        except Exception:
        ...
        if not products:
            ...
            raise RuntimeError(
                f"상품을 찾지 못했습니다.\n"
                f"  목표 URL: {url}\n"
                f"  실제 URL: {current_url}\n"
                f"  HTML 저장: {snapshot_path}"
            )
    ...
    def _wait_for_content(self, scraping_settings: dict) -> None:
        """Hook: <body> 감지 후 실제 콘텐츠가 렌더링될 때까지 추가 대기.
        기본값은 단순 sleep. 브랜드별로 더 정확한 조건으로 오버라이드합니다."""
        ...
        for i, product in enumerate(products):
            ...
            try:
                ...
                try:
                    ...
                    err_log = project_root / "tmp_debug" / "detail_errors.log"
    ...
    def _wait_for_detail_content(self) -> None:
        """Hook: 상세 페이지 주요 컨텐츠 렌더링 대기. 필요 시 브랜드별로 오버라이드."""
        pass
    ...
    def _extract_material(self, product: dict) -> str:
        for key in ("material", "materials", "composition", "fabrication", "finishing"):
            val = product.get(key, "")
            if isinstance(val, str) and val.strip():
                return val.strip()
            if isinstance(val, dict):
                text = val.get("text") or val.get("value") or val.get("label") or ""
                if text:
                    return str(text).strip()
        for container_key in ("characteristics", "attributes"):
            container = product.get(container_key, {})
            if isinstance(container, dict):
                for mat_key in ("material", "materials", "composition"):
                    val = container.get(mat_key, "")
    ...
    def _extract_page_images(self, html: str) -> str:
        ...
        return ", ".join(list(dict.fromkeys(u for u in urls if u))[:10])
    ...
    def _detect_block_reason(self, html: str) -> Optional[str]:
        ...
        # Chrome NTP 또는 브라우저 내부 페이지에 착지한 경우 (네비게이션 실패)
        if "<cr-iconset" in html or "chrome://new-tab-page" in html_lower:
            return "browser_ntp_page"

        if "403 forbidden" in html_lower or "error 403" in html_lower:
            return "http_403_forbidden"
        if "access denied" in html_lower:
            # LV는 "REF #", 일반 Akamai는 "Reference #" 형식을 사용
            if "ref #" in html_lower or "reference #" in html_lower or "akamai" in html_lower:
