...
class LvScraper(BaseScraper):
    ...
    def _wait_for_detail_content(self) -> None:
        try:
            WebDriverWait(self.driver, 10).until(
                lambda d: (
                    d.find_elements(By.CSS_SELECTOR, "p.lv-product__description")
                    or d.find_elements(By.CSS_SELECTOR, "div.lv-product-detailed-features")
                )
            )
        except Exception:
            pass
    ...
    def parse_detail(self, url: str) -> dict:
        ...
        # URL 끝 세그먼트가 SKU (예: .../M46990)
        sku = url.rstrip("/").split("/")[-1]
        if sku and self.driver:
            try:
    ...
    def _fill_from_selectors(self, html: str, result: dict) -> None:
        ...
        # 색상(엑셀): "소재" 변형 버튼 우선 → 없으면 상세 특징 ul 첫 3개 li
    ...
    def _extract_products_from_selectors(self, html: str, category_name: str, category_url: str) -> list[dict]:
        ...
        sku_pattern = re.compile(r"[A-Z][0-9]{4}[0-9A-Z]")
        products = []
